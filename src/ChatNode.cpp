...
void ChatNode::broadcastHello() {
  WireMessage w; w.type="hello"; w.origin=st_.originId; w.chatPort = chatPort_;
...
void ChatNode::handleHello(const QHostAddress& from, quint16 port) {
  addPeer(from, port);
  WireMessage w; w.type="hello_ack"; w.origin=st_.originId; w.chatPort = chatPort_;
...
void ChatNode::onReadyReadDiscovery() {
  while (discSock_.hasPendingDatagrams()) {
    ...
    WireMessage w = unpack(d.data());
    if (w.type=="hello") {
      quint16 theirChat = (w.chatPort > 0 && w.chatPort <= 65535) ? static_cast<quint16>(w.chatPort) : chatPort_;
      handleHello(d.senderAddress(), theirChat);
    }
  }
}
...
void ChatNode::handleMessage(const WireMessage& w, const QHostAddress& from, quint16 port) {
  ...
  if (w.type=="chat") {
    // If this is a direct message and not for our port, ignore completely.
    if (w.dest != -1 && w.dest != chatPort_) return;

    // This message is either broadcast or intended for us.
    sendAck(w.origin, w.seq, from, port);

    // Deliver only once (new highest per-origin)
    if (w.seq > st_.seen.vc.value(w.origin, 0LL)) {
      storeAndDeliver(w);

      // Forward ONLY broadcasts; do not forward direct messages.
      if (w.dest == -1) {
        for (auto& p : peers_.peers)
  ...
  if (w.type=="summary") {
    QVariantMap vm; for (auto it = w.vc.constBegin(); it != w.vc.constEnd(); ++it) vm[it.key()] = it.value();
    VectorClock theirs = VectorClock::fromVariantMap(vm);
